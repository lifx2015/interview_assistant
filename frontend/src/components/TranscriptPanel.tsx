    ...
    <div className={styles['transcript-area']}>
      ...
      {status === 'recording' && currentPartial && (
        <div className={styles['partial-hint']}>
          正在识别: <span className={styles['partial-text']}>{currentPartial}</span>
        </div>
      )}
      ...
      {currentPartial && (
        <div className={`${styles.bubble} ${styles[currentRole]} ${styles.partial}`}>
          <span className={styles['bubble-label']}>{currentRole === 'interviewer' ? '面试官' : '候选人'}</span>
          <span className={`${styles['bubble-text']} typing-cursor`}>{currentPartial}</span>
        </div>
      )}
      <div ref={bottomRef} />
    </div>
